...
func TestMain(m *testing.M) {
	r = NewRedisClient(&Config{Endpoint: "127.0.0.1:6379",PoolSize: 10,Password: "ahffkTlqkf",Database: 0}, prefix, 0, 30000)
	//reset()
	c := m.Run()
	//reset()
	os.Exit(c)
...
func TestAddShared(t *testing.T) {

	//start := time.Now()
	//for i := 100000; i < 10000; i++ {
	//	data := fmt.Sprintf("%06d", i)
	//	r.client.Set(data,data,10000)
	//}
	//fmt.Println(time.Since(start))
	//start := time.Now()
	//for j := 1;j < 100;j++ {
	//	data := fmt.Sprintf("%06d", 10000)
	//	r.client.Keys(data)
	//}
	//fmt.Println(time.Since(start))
	...
	for i := 1; i < 4000; i++ {
		 data := fmt.Sprintf("%06d",i)
		data = "0xd166c49334542e26e5f855e04093d48fff" + data
		r.client.LPush(r.formatKey("lastshares"), data)
	}
	r.client.LTrim(r.formatKey("lastshares"), 0, r.pplns)
}

func TestWriteShareCheckExist(t *testing.T) {
	reset()
	windowRate := util.MustParseDuration("3h")
	exist, _ := r.WriteShare("x", "x", "x", []string{"0x0", "0x0", "0x0"}, 10, 1008, windowRate, "", 0)
	if exist {
		t.Error("PoW must not exist")
	}
	exist, _ = r.WriteShare("x", "x", "x", []string{"0x0", "0x1", "0x0"}, 10, 1008, windowRate, "", 0)
	if exist {
		t.Error("PoW must not exist")
	}
	exist, _ = r.WriteShare("x", "x", "x", []string{"0x0", "0x0", "0x1"}, 100, 1010, windowRate, "", 0)
	if exist {
		t.Error("PoW must not exist")
	}
	exist, _ = r.WriteShare("z", "x", "x", []string{"0x0", "0x0", "0x1"}, 100, 1016, windowRate, "", 0)
	if !exist {
		t.Error("PoW must exist")
	}
	exist, _ = r.WriteShare("x", "x", "x", []string{"0x0", "0x0", "0x1"}, 100, 1025, windowRate, "", 0)
	if exist {
		t.Error("PoW must not exist")
	}
}
